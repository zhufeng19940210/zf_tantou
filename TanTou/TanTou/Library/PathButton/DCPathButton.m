...
- (void)configureViewsLayoutWithButtonSize:(CGSize)centerButtonSize {
    ...
    //
    _bottomView = [[UIView alloc]initWithFrame:CGRectMake(self.bloomSize.width - _bloomRadius, self.bloomSize.height - _bloomRadius, _bloomRadius, _bloomRadius)];
...
//    if (self.isBloom) {
//        [_pathCenterButton setImage:[UIImage imageNamed:@"×"] forState:UIControlStateNormal];
//        [_pathCenterButton setImage:[UIImage imageNamed:@"×"] forState:UIControlStateHighlighted];
//    }else{
//        [_pathCenterButton setImage:[UIImage imageNamed:@"T"] forState:UIControlStateNormal];
//        [_pathCenterButton setImage:[UIImage imageNamed:@"T"] forState:UIControlStateHighlighted];
//    }
}
...
//                             [_pathCenterButton setImage:[UIImage imageNamed:@"T"] forState:UIControlStateHighlighted];
    ...
    [UIView animateWithDuration:0.0618f * 3
                     ...
                     animations:^{
                         outsideView.alpha = 0.0f;
                         insideView.alpha = 0.0f;
                     }
                     completion:nil];
    /****************** kinmo添加 ******************/
...
//                     completion:nil];
    ...
    /****************** kinmo添加 ******************/
    if (outsideView == nil) {
        outsideView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _bloomRadius * 2.6, _bloomRadius * 2.6)];
        outsideView.backgroundColor = [UIColor whiteColor];
        outsideView.alpha = 0.0f;
        outsideView.center = self.pathCenterButtonBloomCenter;
        [self insertSubview:outsideView atIndex:0];
        
        // ----- 使用贝塞尔曲线画圆
        UIBezierPath *outsidePath = [UIBezierPath bezierPathWithOvalInRect:outsideView.bounds];
        CAShapeLayer *outsidePathLayer = [CAShapeLayer layer];
        outsidePathLayer.path = outsidePath.CGPath;
        outsideView.layer.mask = outsidePathLayer;
    }
    
    if (insideView == nil) {
        insideView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _bloomRadius * 1.3, _bloomRadius * 1.3)];
        insideView.backgroundColor = [UIColor whiteColor];
        insideView.alpha = 0.0f;
        insideView.center = self.pathCenterButtonBloomCenter;
        [self insertSubview:insideView aboveSubview:outsideView];
        
        // ----- 使用贝塞尔曲线画圆
        UIBezierPath *insidePath = [UIBezierPath bezierPathWithOvalInRect:insideView.bounds];
        CAShapeLayer *insidePathLayer = [CAShapeLayer layer];
        insidePathLayer.path = insidePath.CGPath;
        insideView.layer.mask = insidePathLayer;
    }
    
    [UIView animateWithDuration:0.0618f
                          delay:0.0f
                        options:UIViewAnimationOptionTransitionNone
                     animations:^{
                         outsideView.alpha = 0.2f;
                         insideView.alpha = 0.7f;
                     }
                     completion:nil];
    /****************** kinmo添加 ******************/
